...
<script>
    ...
    var barChartData = {
      ...
      <?php
            $content1= $this->db->query("select month(tanggal_survey) as 'bulan', avg(ikm) as 'rikm' from survey left join kuesioner on kuesioner.id=survey.id_kuesioner left join jenis_layanan on jenis_layanan.id=kuesioner.id_jenis_layanan left join unit_layanan on unit_layanan.id=jenis_layanan.id_unit_layanan where id_unit_layanan=".$unlay." group by month(tanggal_survey)");
            $i=0;
            foreach ($content1 -> result_array() as $row1): ?>
            <?php $i=$i+1;?>
