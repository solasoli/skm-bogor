...
class Kues extends CI_Controller {
	...
	public function link()
	{
		...
		if (!isset($_SESSION['skm_isLogged'])) {redirect('auth/login','refresh');} else		
		{
			$_SESSION['title']='SKM | Link';
			$_SESSION['side']='link';
			$this->load->view('title');
			$this->load->view('header');
			$this->load->view('sidebar');
			$this->template->load('contentheader','link_hdr');
			$this->template->load('contentjdl','link_jdl');
			$this->template->load('content','link');
			$this->load->view('footer');
		}
	}
	...
	public function kuesioner_add($id = NULL)
	{
		...
		if (!isset($_SESSION['skm_isLogged'])) {redirect('auth/login','refresh');} else		
		{
			$_SESSION['title']='SKM | Kuesioner -> Tambah Data';
			$_SESSION['side']='';
			$this->load->view('title');
			$this->load->view('header');
			$this->load->view('sidebar');
	...
	public function kuesioner_edit( $id = NULL )
	{
		...
		if (!isset($_SESSION['skm_isLogged'])) {redirect('auth/login','refresh');} else		
		{
			$_SESSION['title']='SKM | Kuesioner -> Edit Data';
			$this->load->view('title');
			$this->load->view('header');
			$this->load->view('sidebar');
			$this->template->load('contentheader','kuesioner_hdr');
	...
	public function action_soal_add($id = NULL)
	{
		if ($this -> input -> post('tanya')!='')
		{
			$rnd=rand(1,10000000000);
	...
	public function soal_delete( $id = NULL )
	{
